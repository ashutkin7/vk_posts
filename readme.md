Вот обновленная версия вашего файла `readme.md`, в которую внесены актуальные доступы к базе данных, указание схемы `clustering_article` и добавлены новые мультимодальные поля (`photo_urls`, `video_urls`) в структуру таблицы постов.

Для работы с библиотекой `SQLAlchemy` параметр схемы передается через `connect_args`.
...
DB_PORT = '5432'
DB_NAME = 'neuro'
SCHEMA_NAME = 'clustering_article'
...
# Обязательно передаем параметр схемы по умолчанию
engine = create_engine(
    connection_str, 
    connect_args={'options': f'-c search_path={SCHEMA_NAME}'}
)

print(f"✅ Подключение к БД установлено. Используется схема: {SCHEMA_NAME}")
...
# ------------------------------------------------------------------
# СЦЕНАРИЙ 2: Получение текстов постов и ссылок на медиа для кластеризации
# ------------------------------------------------------------------
def get_posts_dataframe(limit=5000):
    """
    Выгружает посты, где есть текст длиннее 10 символов,
    а также захватывает прямые ссылки на фото и видео для дальнейшего анализа.
    """
    query = """
    SELECT
        owner_id,
...
if __name__ == "__main__":
    ...
    print(df_posts[['text', 'photo_urls', 'likes_count']].head()) # Вывод первых строк
    ...
    # Пример: Распределение по полу (1 - Женский, 2 - Мужской)
...
## Cтруктура базы данных (Схема: `clustering_article`)
...
| **`photo_urls`** | `Text` | Прямые ссылки на фото (разделённые запятой). |
| **`video_urls`** | `Text` | Прямые ссылки на видео (разделённые запятой). |
| **`attachments`** | `JSONB` | Массив медиа-вложений (фото, видео, ссылки, опросы). |
| **`scraped_at`** | `Timestamp` | Время сбора. |

*Primary Key:* Составной ключ `(owner_id, id)`.
...
Хотите, я также набросаю базовый скрипт, использующий `EasyOCR` для автоматического извлечения текста из тех `photo_urls`, которые мы теперь сохраняем?